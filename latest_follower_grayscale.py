...
class LineFollower2:
    ...
    def _generate_error(self, l_cs_tol: float,
                        r_cs_tol: float):
        lr, lg, lb = rgb(self._l_cs.value(0), self._l_cs.value(1), self._l_cs.value(2), self._l_cs.red_max, self._l_cs.green_max, self._l_cs.blue_max)
        rr, rg, rb = rgb(self._r_cs.value(0), self._r_cs.value(1), self._r_cs.value(2), self._r_cs.red_max, self._r_cs.green_max, self._r_cs.blue_max)
        ...
        error = (left_gray*l_cs_tol - right_gray*r_cs_tol)
    ...
    def follow_line_for_time(self, speed, follow_time, sleep_time=0.01,
        ...
        while not self.button.on_enter():
            pass
...
if __name__ == "__main__":
    regulator = PID(kp=1.5, ki=0.01, kd=0.5, integral_reset_count=5)
    follower = LineFollower2(OUTPUT_C, OUTPUT_B, INPUT_2, INPUT_1, regulator,
                             False, False)
    try:
        follower.follow_line_for_time_raw(speed=8, follow_time=120, sleep_time=0.001,
